...
module.exports = {
  ...
  activate(state) {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      algorithm: "command-t",
      confirmAction: "fuzzy-explorer:open",
      additionalActionCommands: ["fuzzy-explorer:edit", "fuzzy-explorer:clear-recent"],
      ...
      actionsFilter: ({ name }) => {
        if (name === "fuzzy-explorer:clear-recent") return this.recentlyUsed.length > 0;
        return (
  ...
  elementForItem(item, { highlight }) {
    return {
      ...
      didRender: (li) => {
        ...
        li.addEventListener("click", (event) => this.openExternalOnAltClick(event, item));
      },
    };
  },
  ...
  updateQueryFromItem() {
    ...
    this.selectList.getQueryEditor().setText(text);
